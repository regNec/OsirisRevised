        ...
        for pc in pcs:
            for arithmetic_error in arithmetic_errors:
                if arithmetic_error["pc"] == pc:
                    ...
                    vul_results[hex(pc)] = arithmetic_error["instruction"].opcode.lower()
...
def closing_message():
    ...
    log.info("Got results: %s.", str(vul_results))
    results['vul'] = vul_results
    if global_params.STORE_RESULT:
        ...
        elif '.hex.evm.disasm' in c_name:
            result_file = os.path.join(global_params.RESULTS_DIR, c_name.replace('.hex.evm.disasm', '.json').split('/')[-1])
        if not os.path.isfile(result_file):
            with open(result_file, 'wt') as of:
                if ':' in c_name:
                    of.write("{")
